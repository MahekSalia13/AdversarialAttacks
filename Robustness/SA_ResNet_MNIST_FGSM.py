...
save_dir = os.path.join("save", f"{MODEL_NAME.lower()}_{DATASET_NAME.lower()}")
destination_path = os.path.join(
    save_dir,
    f"{MODEL_NAME.lower()}_{DATASET_NAME.lower()}_"
)
    ...
    for epoch in range(EPOCHS):
        ...
        for indx, (inputs, labels) in enumerate(data_loaders['train']):
            try:
                ...
                running_loss += batch_loss_total  # cumulative sum of loss
    ...
    def move(self):
        ...
        for i in range(moves_count):
            ...
            if loc in self.p1:  # the first part, which is for the operator
                ...
                if self.state[loc] < 2:  # if operator is a.f(x), remove the second activation function
                ...
                self.state[loc] = random.randint(0, len(Activation_Functions) - 1)
                if self.state[loc - LAYERS * 2] < 2:  # if operator is a.f(x), no need for second activation
